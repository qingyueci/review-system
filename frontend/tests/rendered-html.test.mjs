...
test("首页日期会跨日自动刷新且保留手动选择的历史日期", () => {
  assert.match(html, /window\.setInterval\(refreshDate, 30_000\)/);
  assert.match(html, /selectedDate === previousDate \? nextDate : selectedDate/);
  assert.match(html, /max=\{currentDate\}/);
});
